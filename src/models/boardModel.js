...
const getDetails = async (id) => {
  try {
    ...
    // const result = await GET_DB().collection(BOAD_COLLECTION_NAME).findOne({ _id: new ObjectId(id) })
    const result = await GET_DB().collection(BOAD_COLLECTION_NAME).aggregate([
      {
        $match: {
          _id: new ObjectId(id),
          _destroy: false
